...
public class ObstacleManager {
    ...
    private  float m_speed = Constants.SCREEN_HEIGHT / 10000.0f;
    ...
    private void fillObstacles() {
        int current_y = (-5 * Constants.SCREEN_HEIGHT) / 4;
        // while the last obstacle
        while (current_y < 0) {
            int random_start_x = (int) (Math.random() * (Constants.SCREEN_WIDTH - this.m_player_gap));
    ...
    public void update() {
        ...
        if (this.m_obstacle_array.get(this.m_obstacle_array.size() - 1).getShape().top >= Constants.SCREEN_HEIGHT) {
            int random_start_x = (int) (Math.random() * (Constants.SCREEN_WIDTH - this.m_player_gap));
